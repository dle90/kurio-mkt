...
**ROAS attribution methodology (added 2026-05-19, important):**
- 📐 **Use ORDER-BASED revenue (sum approved `sale_orders.real_amount` by `created_at` month, attribute via order.account_phone → sheet first-touch → code).** The earlier `account.total_revenue` cohort method was wrong: it credited a customer's lifetime revenue to the month their Getfly account was created, which inflated early-month ROAS and deflated late-month ROAS. Scripts: `src/roas/fetch_getfly_orders_ytd.js`, `src/roas/recompute_codes_ranked.js`, `src/roas/cohort_and_active.js`.
...
- ✅ Monthly blended ROAS YTD 2026 (order-based, code-attributed): Jan 0.62, Feb 0.60, Mar 0.72, Apr 0.79, May 0.83 (still maturing). **Trend is up, not down.** Meta-attributable share of all Getfly revenue grew 28% (Jan) → 52% (May).
